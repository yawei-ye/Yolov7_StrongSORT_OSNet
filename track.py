...
def run(
    ...
    for frame_idx, (path, im, im0s, vid_cap) in enumerate(dataset):
        ...
        pred = non_max_suppression(
            pred[0], conf_thres, iou_thres, classes, agnostic_nms
        )
    ...
    print(
        f"Speed: %.1fms pre-process, %.1fms inference, %.1fms NMS, %.1fms strong sort update per image at shape {(1, 3, imgsz, imgsz)}"
        % t
    )
